...
def cadastrar_disciplina(request):
    ...
    with connection.cursor() as cursor2:
        professores = []
        cursor2.execute("SELECT cpf, nome FROM professor")
        for professor in cursor2.fetchall():
            professores.append({"cpf":professor[0],"nome":professor[1]})
            ...
            for letra in ['A', 'B', 'C', 'D']:
                ...
                if opcao_texto:
                    with connection.cursor() as cursor3:
                        cursor3.execute("SELECT cod FROM q_objetiva")
...
def cadastrar_avaliacao(request):
    if request.method == 'POST':
        ...
        for questao_id in questao_ids:
            ...
            with connection.cursor() as cursor2:
                cursor2.execute("SELECT cod FROM avaliacao")
                cursor2.execute("INSERT INTO questoes_usadas (a_cod, q_cod, q_tipo, valor) VALUES (%s, %s, %s, %s)", [valor_total, questao[0],questao[1], valor])
    ...
    with connection.cursor() as cursor3:
        ...
        cursor3.execute("SELECT cpf, nome FROM professor")
        for professor in cursor3.fetchall():
            professores.append({"cpf":professor[0],"nome":professor[1]})
        
        cursor3.execute("SELECT cod, nome FROM disciplina")
        for disciplina in cursor3.fetchall():
            disciplinas.append({"cod":disciplina[0],"nome":disciplina[1]})

        cursor3.execute("SELECT * FROM q_descritiva UNION ALL SELECT * FROM q_objetiva")
        for questao in cursor3.fetchall():
            questões.append({"cod":questao[0],"tipo":questao[1],"pergunta":questao[2]})
...
def lista_alunos(request):
    with connection.cursor() as cursor:
        ...
        for aluno in cursor.fetchall():
            print(aluno)
            alunos.append({"cpf":aluno[0],"nome":aluno[1],
                       "aluno":aluno[2],"semestre":aluno[3]})
...
def lista_professores(request):
    with connection.cursor() as cursor:
        ...
        for professor in cursor.fetchall():
            print(professor)
            professores.append({"cpf":professor[0],"nome":professor[1],"data_nasc":professor[2],
                            "salario":professor[3],"idade":professor[4]})
...
def lista_disciplinas(request):
    with connection.cursor() as cursor:
        ...
        for disciplina in cursor.fetchall():
            disciplinas.append({"cod":disciplina[0],"nome":disciplina[2],
                                "descricao":disciplina[3],"p_cpf":disciplina[1]})
...
def lista_questoes(request):
    with connection.cursor() as cursor:
        questões = []
        opcoes = []
        cursor.execute("SELECT * FROM q_descritiva UNION ALL SELECT * FROM q_objetiva")
        for questao in cursor.fetchall():
            print(questao)
            if questao[1] == 'OBJETIVA':
                print(questao[0])
                questões.append({"cod":questao[0],"tipo":questao[1],"pergunta":questao[2],"resposta_correta":questao[3]})
                cursor.execute("SELECT opcao, letra FROM opcoes WHERE cod = %s ORDER BY letra ASC", str(questao[0]))
                for opcao in cursor.fetchall():
                    opcoes.append({"letra":opcao[1],"opcao_texto":opcao[0]})
            else:
                questões.append({"cod":questao[0],"tipo":questao[1],"pergunta":questao[2],"resposta_esperada":questao[3]})
            print("%s%s", str(questao[0]), questao[1])                       
